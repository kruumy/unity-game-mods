...
public class RCC_CustomizerExample : MonoBehaviour
{
	...
	public static RCC_CustomizerExample Instance
	{
		get
		{
			if (instance == null)
			{
				instance = Object.FindObjectOfType<RCC_CustomizerExample>();
			}
			return instance;
		}
	}
	...
	{
		RCC_Customization.SetGearShiftingThreshold(RCC_SceneManager.Instance.activePlayerVehicle, Mathf.Clamp(slider.value, 0.5f, 0.95f));
	}

	public void SetClutchThresholdBySlider(Slider slider)
	{
		RCC_Customization.SetClutchThreshold(RCC_SceneManager.Instance.activePlayerVehicle, Mathf.Clamp(slider.value, 0.1f, 0.9f));
	}
	...
	public void SaveStatsTemp()
	{
		RCC_Customization.SaveStatsTemp(RCC_SceneManager.Instance.activePlayerVehicle);
	}

	public void LoadStatsTemp()
	{
		RCC_Customization.LoadStatsTemp(RCC_SceneManager.Instance.activePlayerVehicle);
		CheckUIs();
	}

	public void ResetStats()
	{
		RCC_Customization.ResetStats(RCC_SceneManager.Instance.activePlayerVehicle, Object.FindObjectOfType<RCC_PhotonDemo>().selectableVehicles[PlayerPrefs.GetInt("DROPDOWNSELECTOR")]);
		Debug.Log(Object.FindObjectOfType<RCC_PhotonDemo>().selectableVehicles[PlayerPrefs.GetInt("DROPDOWNSELECTOR")]);
		CheckUIs();
	}
