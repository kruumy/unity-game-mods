...
#nullable disable
        ...
        public Sun() : base(nameof(Sun), SubPanel.Graphics)
        {
            SceneManager.activeSceneChanged += ( Scene _, Scene _ ) => { CopySun(); };

            Override = CreateMenuCheckBoxWithResetToDefault(false, "Override Sun", "Override the current stage's sun. Some shadow settings above will NOT apply until the next stage is loaded or the recopy sun button is pressed. This is because the sun object gets copied at the start of the stage.", true, ( bool newValue ) =>
            {
            ...
            Intensity = CreateMenuSliderWithResetToDefault(1, 10, 0, false, "Sun Intensity", string.Empty, true, ( float newValue ) =>
            {
                if ( NewSun != null )
                {
            ...
            Red = CreateMenuSliderWithResetToDefault(1, 1, 0, false, "Sun Color Red", string.Empty, true, ( float newValue ) =>
            {
                if ( NewSun != null )
                {
            ...
            Green = CreateMenuSliderWithResetToDefault(1, 1, 0, false, "Sun Color Green", string.Empty, true, ( float newValue ) =>
            {
                if ( NewSun != null )
                {
            ...
            Blue = CreateMenuSliderWithResetToDefault(1, 1, 0, false, "Sun Color Blue", string.Empty, true, ( float newValue ) =>
            {
                if ( NewSun != null )
                {
            ...
            ShadowSoftness = CreateMenuSliderWithResetToDefault(1, 35, 0, false, "Sun Shadow Softness", string.Empty, true, ( float newValue ) =>
            {
                if ( NewSunNGSS != null )
                {
